...
DISTRO_NAMES = ['arch', 'centos', 'debian', 'fedora', 'freebsd', 'gentoo',
                'opensuse', 'redhat', 'rhel', 'sles', 'suse', 'ubuntu', 'rocky']
...
def os_release(target=None):
    ...
    if not data:
        for relfile in [target_path(target, rel) for rel in
                        ['etc/centos-release', 'etc/redhat-release', 'etc/rocky-release']]:
...
def is_rocky(target=None):
    """Check if Rocky specific file is present at target"""
    return os.path.exists(target_path(target, 'etc/rocky-release'))
